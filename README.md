...
- **官方 Speaking 三段独立训练**：Part 1、Part 2、Part 3 使用 IELTS.org 公开样题中的 hometown、important possession、status and values 主题。每个 Part 包含一组连续问题：用户提交当前语音后，考官会自动读出下一道追问，并重新进入准备与录音流程；只有回答完整组问题，才把该 Part 标记完成。作答前隐藏改进建议、话题模板、官方样题页和示范录音，避免用户提前照着答案说；整组完成后才汇总每轮录音的时长、音量、低音量区间、削波与语音识别稿，解锁语音不足分析、3 个针对性改进练习、荧光识别稿，以及可替换的话题结构与示例句。正式考试只有 Part 2 固定提供 1 分钟准备，Part 1/3 的 60 秒在界面中明确标为 App 强化训练模式。三个 Part 全部完成后才记录整套完成。
