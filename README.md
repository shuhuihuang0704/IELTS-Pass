...
2. 精听一段租房咨询并完成题目。
3. 进入口语训练，与 AI 考官围绕住房与城市进行更抽象的双向讨论。
...
- **口语模拟**：点击开始后考官才用语音提问，播放中可以暂停并从原位置继续，也可以从头重听；字幕默认隐藏，听不懂时可以显示；支持麦克风语音识别与文字输入，识别后由用户确认并提交回答。
...
- **进度激励与真实停留计时**：今日完成度、连续学习、本周时长、各项完成状态；页面在前台可见时累计学习秒数，每 10 秒保存一次，切到后台、锁屏或关闭页面时暂停并立即保存，不再用“完成某任务约等于若干分钟”的估算值。点击首页连续学习卡片，可查看最近 7 天柱状图、每日学习折线趋势、最近 6 个月累计时间，以及每天完成的具体内容。
- **今日加练**：完成词汇、听力、口语和阅读四项基础任务后，首页出现“继续增加学习”；可继续选择复习、听力、口语、阅读或套题训练，基础完成度保持 100%，额外停留时间继续计入当天。
...
- **Android 可安装版本**：安卓用户可通过 Chrome 打开公开网址并选择“安装”，添加到桌面后以独立 App 窗口运行；账号与网页版本共用。
...
5. 切换到口语，用文字或麦克风回答考官；先提交一个很短的回答，检查考官追问，再提交完整回答。
