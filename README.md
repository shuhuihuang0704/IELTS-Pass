...
- **官方 Speaking 三段独立训练**：Part 1、Part 2、Part 3 使用 IELTS.org 公开样题中的 hometown、important possession、status and values 主题，每个 Part 都独立完成“考官语音提问 → 60 秒准备 → 浏览器录音 → 暂停/继续 → 回听/重录 → 提交”的闭环。作答前隐藏改进建议、话题模板、官方样题页和示范录音，避免用户提前照着答案说；提交后才依据实际录音时长、音量、低音量区间、削波与语音识别稿，解锁语音不足分析、3 个针对性改进练习、荧光识别稿，以及可替换的话题结构、示例句和后续追问。正式考试只有 Part 2 固定提供 1 分钟准备，Part 1/3 的 60 秒在界面中明确标为 App 强化训练模式。三个 Part 全部提交后才记录本套完成。
