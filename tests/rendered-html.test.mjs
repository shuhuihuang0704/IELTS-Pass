...
test("ships all four learning modes and persistent progress", async () => {
  ...
  assert.match(state, /listening: \{ rate: \.92, passScore: 7/);
  assert.match(state, /reading: \{ minutes: 15, passScore: 10/);
  assert.match(state, /speaking: \{ minimumWords: 40, requireDevelopment: true, requireCounterpoint: true/);
  assert.match(app, /DAILY DIFFICULTY/);
